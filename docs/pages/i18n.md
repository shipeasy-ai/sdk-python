...
# Every argument is optional — the key, profile and CDN origin come from
# configure(client_key=..., profile="{{PROFILE}}").
head = shipeasy.i18n_script_tag()
```

`i18n_script_tag(client_key=None, profile=None, base_url=None)` returns the
loader `<script>` tag; pass an argument only to override the configured value
for that one tag. You can also fold the i18n tag into the flags bootstrap tag:
