...
Shipeasy doc standard (experiment-platform `21-tooling-monorepo.md` §A4.4) so the
...
```
https://shipeasy-ai.github.io/sdk-python/manifest.json
https://shipeasy-ai.github.io/sdk-python/pages/<page>.md
https://shipeasy-ai.github.io/sdk-python/snippets/<group>/<leaf>.md
...
- `placeholders` — every `{{TOKEN}}` used in snippets (callers substitute these).
  ...
  `EVENT_NAME`, `SUCCESS_EVENT`, `PROFILE`. Add a token here when you introduce it.
...
SDK's real API**. Written around `configure()` + `shipeasy.Client(user)` — never
the `Engine` (see the repo-root `CLAUDE.md`).
...
- **Construct the bound client on its own line**, with a "construct once per
  callsite" comment — never chain `Client(user).get_flag(...)`.
