...
class GetMetricSeriesResponse(BaseModel):
    ...
    rows: List[GetMetricSeriesResponseRowsInner] = Field(description="Bucketed series, ordered by `t` ascending. For a metric with a `groupBy` clause each bucket has one row per label combination, with the label values as extra string/number properties on the row.")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GetMetricSeriesResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GetMetricSeriesResponse from a dict"""
        if obj is None:
            return None
