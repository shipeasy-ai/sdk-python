...
class CreateGateRequest(BaseModel):
    ...
    enabled: StrictBool = Field(description="Master switch. Defaults to `true`. Set `false` to create the gate disabled (evaluates to `false` regardless of rules/rollout); flip on via `POST /{id}/enable` or PATCH.")
    rollout_pct: Annotated[int, Field(le=10000, strict=True, ge=0)] = Field(description="Initial rollout in **basis points** (0–10000 = 0%–100%) — `100` here means **1%**, not 100%. Use `rollout_percent` (0–100) below if you'd rather think in percent. Use `0` to create the gate dark and ramp via PATCH after deploy validation.")
    ...
    rules: List[ListGatesResponseDataInnerRulesInner] = Field(description="Targeting predicates. AND-combined. If non-empty, the gate returns `true` only for callers that satisfy every rule **and** fall under `rollout_pct`.")
