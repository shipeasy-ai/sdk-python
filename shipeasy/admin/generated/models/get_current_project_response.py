...
class GetCurrentProjectResponse(BaseModel):
    ...
    error_autoclose_days: StrictInt = Field(description="Days an `open` tracked error may go unseen before the nightly sweep auto-resolves it. `0` disables auto-close for this project. Ingest reopens a resolved error the moment it recurs, so this never hides a live issue.", alias="errorAutocloseDays")
    error_ticket_min_occurrences: StrictInt = Field(description="Occurrence count a tracked error must cross before an `error` ticket is auto-filed into the ops queue. Customising it requires a paid plan; unentitled projects always run at the platform default of 10.", alias="errorTicketMinOccurrences")
