...
class FireConnectorRequest(BaseModel):
    """
    Body for `POST /api/admin/connectors/{id}/fire`. Entirely optional — an empty body kicks the run with the provider default.
    """ # noqa: E501
    text: Optional[StrictStr] = Field(default=None, description="Optional prompt override for the run. When omitted, each provider falls back to its own default (Claude → the configured `fireText`; the others → their built cold-start prompt).")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of FireConnectorRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of FireConnectorRequest from a dict"""
        if obj is None:
            return None
