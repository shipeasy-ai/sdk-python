...
class CreateExperimentRequestGoalMetric(BaseModel):
    """
    ...
    query: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=4096)]] = Field(default=None, description="Metric DSL string, e.g. `count_users(checkout_completed)`. Provide this OR `event`.")
    event: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=256)]] = Field(default=None, description="Event name to build the metric from server-side (friendlier alternative to `query`). Auto-created if missing.")
    aggregation: Optional[StrictStr] = Field(default=None, description="Reducer for the `event` form. Defaults to `count_users`.")
    value: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=256)]] = Field(default=None, description="Numeric event property for `sum`/`avg` aggregations (with `event`).")
    ...
    def aggregation_validate_enum(cls, value):
        ...
        if value not in set(['count_users', 'count_events', 'retention_7d', 'retention_30d', 'sum', 'avg']):
            raise ValueError("must be one of enum values ('count_users', 'count_events', 'retention_7d', 'retention_30d', 'sum', 'avg')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CreateExperimentRequestGoalMetric from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CreateExperimentRequestGoalMetric from a dict"""
        if obj is None:
            return None
