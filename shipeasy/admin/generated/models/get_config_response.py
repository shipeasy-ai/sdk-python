...
class GetConfigResponse(BaseModel):
    ...
    creator_email: Optional[StrictStr] = Field(default=None, description="Resolved email of the creator (`created_by` → `users.email`), or `null` when unknown. List-only enrichment — present on `GET /api/admin/configs` rows, absent from this by-id detail — so it is optional, never required.", alias="creatorEmail")
    created_at: Optional[StrictStr] = Field(default=None, description="ISO-8601 creation timestamp, or `null` on legacy rows. List-only enrichment — present on `GET /api/admin/configs` rows, absent from this by-id detail — so it is optional, never required.", alias="createdAt")
