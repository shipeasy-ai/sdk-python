...
class ExperimentInlineMetric(BaseModel):
    """
    Inline metric — a DSL `query`, or an `event` (+ `aggregation`/`value`) the server compiles into one.
    """ # noqa: E501
    name: Optional[Annotated[str, Field(strict=True, max_length=128)]] = Field(default=None, description="Stable metric key. Single segment or `folder.name`; lowercase letters, digits, `_`/`-`; max 128 chars.")
    query: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=4096)]] = Field(default=None, description="Metric DSL string. Provide this OR `event`.")
    ...
    aggregation: Optional[StrictStr] = Field(default=None, description="Reducer for the `event` form. Defaults to `count_events`. `count_users` was removed: Analytics Engine samples rows and weights the survivors, and a distinct count cannot be reweighted, so it under-reports by the sample interval. `retention_7d` / `retention_30d` were removed on 2026-08-10: the day count reached no query and no reducer, so both produced the same number as a distinct user count and reported it as a returning-user rate.")
    ...
    def aggregation_validate_enum(cls, value):
        ...
        if value not in set(['count_events', 'sum', 'avg']):
            raise ValueError("must be one of enum values ('count_events', 'sum', 'avg')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ExperimentInlineMetric from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ExperimentInlineMetric from a dict"""
        if obj is None:
            return None
